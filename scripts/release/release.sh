...
publish() {
  ...
  log "Publishing dhruvinparikh-defi-legos on npm"
  npm publish --tag "$dist_tag" --otp "$(prompt_otp)"

  log "Publishing @dhruvinparikh/contracts on npm"
  cd contracts
  ...
  if [[ "$dist_tag" == "latest" ]]; then
    otp="$(prompt_otp)"
    npm dist-tag rm --otp "$otp" dhruvinparikh-defi-legos next
    npm dist-tag rm --otp "$otp" @dhruvinparikh/contracts next
  fi
}
